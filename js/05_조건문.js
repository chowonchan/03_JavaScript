...
function check1(){
  ...
  // input1.value : inpu1 요소에 입력된 값 (string)
  // Number(문자열) : 문자열 -> 숫자로 변경
  const value = Number(input1.value);

  // 입력 받은 값이 0보다 큰경우 true(라는 조건식 작성)
  if(value > 0){
    alert(`${value} 은/는 양수 입니다.`);
  }

  // 입력 받은 값이 0보다 작거나 같은경우
  // == 양수가 아닌 경우
  if(value <= 0){
...
function ageCheck(){
  ...
  // alert(`${value}세는 ${result}`)
  alert(result)
}

// 실습 문제 다른 방식 (넓은 범위 부터 작성해서 코드 문장이 짧아짐)
function ageCheck2(){
  const inputAge = document.getElementById("inputAge")
  const value = Number(inputAge.value);
...
function calc(op){
  ...
  /* break : 현재 case까지만 해석하고 switch문을 멈춤
    -> break 미작성시 
        현재 case 수행후 멈추지 않고 break가 존재하는 case문까지 계속 수행
