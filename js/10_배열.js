...
function check5(){
  ...
  for(let i = 0; i < leng; i++){
    ...
    // 0.0< Math.random <1.0 이 floor로 소수점 제거
    // 0 이상, leng 미만의 난수 발생
...
function check6(){
  ...
  // 배열 내 숫자 정렬 방법
  lotto.sort( function(a,b) {return a-b} ); // 오름 차순 정렬
...
/** 점심 메뉴 뽑기 */
function selectMenu(){
  const menus = ["라면 + 김밥", "제육볶음", "돈까스", "국밥", "맘스터치"
                ,"알탕", "샐러드", "KFC","냉면","쭈꾸미","초밥","고등어구이"];

  // meus 배열에서 랜덤으로 하나 뽑아서
...
function check8(){
  ...
  const search = Number(prompt('찾을려는 값 입력'));
  ...
  for(let row = 0; row < arr.length; row++){
    ...
    for(let col = 0; col < arr[row].length; col++){
      ...
      if(arr[row][col] === search){
        ...
        console.log(`${row}행 ${col}열에 ${search}가 존재 합니다`);
