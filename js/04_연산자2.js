...
function oddEvenCheck(){
  ...
  // 2로 나눴을 때 나머지가 1이 아닌경우 === 짝수

  // 2로 나눴을 때 나머지가 1 ===  홀수
  // 2로 나눴을때 나머지가 0이 아닌경우 === 홀수
...
function multipleCheck(){
  ...
  // 10은 5배수? -> 10을 5로 나눴을 떄 나머지가 0인지 확인
...
/* 
  ...
  OR (||) : 둘다 false인 경우에만 false(나머진 true)
*/

/* AND 연산자 확인 */
function andCheck(){
  // 104 는 100 이상의 숫자 이면서 짝수인가?
  const bool1 = (104 >= 100) && (104 % 2 === 0);

  console.log(`104 는 100 이상의 숫자 이면서 짝수인가? ${bool1}`);
