...
test1a.onclick = function(){
  ...
  // 고전이벤트 모델 - 이벤트 핸들러 내에 this : 클릭된 요소
  console.log(this);
...
test2.addEventListener("click", function(){
  ...
  */
  ...
  // 1) 현재 요소에 inline style로 작성된 투명도 얻어오기 (current)
  ...
  if( test2.style.opacity < 0 ){ // curr < 0
    test2.style.opacity = 1;
  }

})

/* #test2 클릭시 숫자 증가하는 이벤트 추가 */
test2.addEventListener("click", function(){
...
for(let i = 0; i < btns.length; i++){
  ...
  btns[i].addEventListener("click", function(){
    ...
    // #result3의 배경색을color 변수에 저장된 값으로 변경
