...
console.log("number1 변수에 저장된 값 : ", number2);

console.log("number1 + number2 : ", number1 + number2);

// 선언된 변수는 콘솔에서도 사용 가능

// (에러) number3 is not defind
// -> number3라는 변수가 정의 되지 않았다
// == number3 선언 안됨 / 존재 x
...
/* 3. const */
/* 상수(constant, 항상 같은 수), 상수, 변수명 중복 x, 블록{} 레벨 범위 */
...
function constCheck(){
  ...
  // 02_변수와자료형.js:91:16 -> 91 번째 줄에서 오류 발생
...
/* 선수 지식
  1. 전역 변수 : 어디서든 사용 가능 한 변수

  2. 지역 변수 : 특정 지역에서 사용 가능한 변수
    1) 함수 레벨 범위
    -function 함수명(){함수 레벨 범위} 
      함수 정의 부분에서 {} 내부에서
...
function scopeTest(){
  ...
  if(num3 == 300){
    let num4 = 400; // 블록 {} 레벨 지역변수 
                    // == 블록을 벗어나면 사용할 수 없다
                    // == 블록 내부에서만 사용가능
    console.log(" if 내부 num3 : ", num4);
  }

  console.log("num4 : ", num4);
  // num3 is not defined -> num4 가 정의 되지 않음
...
function scopeTest2(){
  ...
  if(1 == 1){// 1과 1이 같다면 if {} 수행
    ...
    // 2) 중복되는 변수 사용시 현재 범위의 변수가 호출된다
  }

  console.log("if 실행후 temp : ",temp);

  console.log("if 실행후 fn2 : ",fn2);
